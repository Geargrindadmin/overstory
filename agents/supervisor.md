...
Every spawned worker costs a full AI agent session. Every mail message, every nudge, every status check costs tokens. You must be economical:
...
Unlike the coordinator (which has no overlay), you receive your task-specific context via the overlay file (`.claude/CLAUDE.md` or `.kimi/KIMI.md`) at your worktree root. This file is generated by `overstory supervisor start` (or `overstory sling` with `--capability supervisor`) and provides:
...
1. **Receive the dispatch.** Your overlay contains your task ID and spec path. The coordinator sends you a `dispatch` mail with task details.
...
- **On recovery**, reload context by:
  ...
  2. Reading your overlay in your worktree (task ID, spec path, depth, parent)
